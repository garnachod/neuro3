...
class RedNeuronalTemporal(Clasificador):
	"""docstring for RedNeuronalTemporal"""
	def __init__(self):
		super(RedNeuronalTemporal, self).__init__()
		self.clases = []
		self.columnas = []
		self.nColumnas = 0
		self.nInstaces = 0
		...
		for epoca in range(0, self.nEpocas):
			...
			for instancia in instancias:
				...
				for indNeurona in range(0, self.nClases):
					...
					#aplicamos identidad a la salida, al ser un problema de regresión
					salidaFinal.append(suma)
				...
				#paso 6 (la derivada ahora es 1)
				deltaMinusculaK = map(lambda x, y: (x - y), vectoresObjetivos[instancia], salidaFinal)
	...
	def classifyInstance(self, instancia):
		return self.computeInstance(instancia)
